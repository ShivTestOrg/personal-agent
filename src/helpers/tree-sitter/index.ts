...
async function parseFile(filePath: string, languageParsers: LanguageParser): Promise<string | undefined> {
  ...
  try {
    ...
    captures.sort(
      (a: { node: { startPosition: { row: number } } }, b: { node: { startPosition: { row: number } } }) => a.node.startPosition.row - b.node.startPosition.row
    );
    ...
    captures.forEach((capture: { node: any; name: any }) => {
